...
export default function App() {
  const [employees, setEmployees] = useLS("ww4_employees", INITIAL_EMPLOYEES);
  const [clothes, setClothes] = useLS("ww4_clothes", INITIAL_CLOTHES);
  const [transactions, setTransactions] = useLS("ww4_transactions", INITIAL_TRANSACTIONS);
  const [auditLog, setAuditLog] = useLS("ww4_audit", []);
      ...
      {tab === "daily" && (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            ...
            )} style={btnSecondary}>📊 ייצוא Excel</button>
            <button onClick={() => exportToPrint("דוח יומי – חוסרים",
              `<table><tr><th>עובד</th><th>מחלקה</th><th>סוג בגד</th><th>ברקוד</th></tr>${missing.flatMap(({emp,items})=>items.map(c=>`<tr><td>${emp.name}</td><td>${emp.department}</td><td>${c.type} ${c.size}</td><td>${c.barcode}</td></tr>`)).join("")}</table>`
            )} style={btnSecondary}>🖨️ ייצוא PDF</button>
          </div>
      ...
      {tab === "history" && (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            <button onClick={() => exportToPrint("היסטוריית תנועות",
              `<table><tr><th>תאריך</th><th>עובד</th><th>ברקוד</th><th>סוג</th><th>פעולה</th></tr>${[...transactions].reverse().map(t=>{const emp=employees.find(e=>e.id===t.employeeId);const c=clothes.find(x=>x.id===t.clotheId);return`<tr><td>${new Date(t.datetime).toLocaleString("he-IL")}</td><td>${emp?.name||""}</td><td>${c?.barcode||""}</td><td>${c?.type||""}</td><td>${t.action}</td></tr>`;}).join("")}</table>`
            )} style={btnSecondary}>🖨️ ייצוא PDF</button>
          </div>
        <div style={tableWrap}>
          <table style={tableStyle}>
...
function exportToPrint(title, tableHtml) {
  const win = window.open("", "_blank");
  win.document.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>${title}</title>
  <style>body{font-family:Arial,sans-serif;direction:rtl;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:right}th{background:#f0f0f0}h2{color:#333}@media print{button{display:none}}</style>
  </head><body><h2>${title}</h2>${tableHtml}<br><button onclick="window.print()">🖨️ הדפס / שמור PDF</button></body></html>`);
  win.document.close();
}
