...
const INITIAL_EMPLOYEES = [
  { id: 1, employeeNum: "E001", uid: "MSH", name: "משה כהן", department: "ייצור", phone: "050-1234567", type: "permanent", status: "active" },
  { id: 2, employeeNum: "E002", uid: "DVL", name: "דוד לוי", department: "אחסנה", phone: "052-7654321", type: "permanent", status: "active" },
  { id: 3, employeeNum: "E003", uid: "RCH", name: "רחל ישראלי", department: "ייצור", phone: "054-1111222", type: "permanent", status: "active" },
  { id: 4, employeeNum: "T001", uid: "YSF", name: "יוסף אברהם", department: "", phone: "", type: "temporary", status: "active" },
];
...
const VALID_STATUS = ["במלאי", "נמסר לעובד", "תקול", "אבוד", "הושמד"];
const STATUS_COLORS = {
  "במלאי":       { bg: "#E8F5E9", text: "#2E7D32", dot: "#4CAF50" },
  "נמסר לעובד": { bg: "#E3F2FD", text: "#1565C0", dot: "#2196F3" },
  "הוחזר":      { bg: "#E8F5E9", text: "#2E7D32", dot: "#4CAF50" },
  "בכביסה":     { bg: "#E8F5E9", text: "#2E7D32", dot: "#4CAF50" },
...
const USERS = [
  { id: "editor", username: "shimi", password: process.env.REACT_APP_ADMIN_PASS || "3155", name: "אחראי המערכת", role: "editor" },
...
function validateScan(barcode, clothes, employees, transactions, mode, scannedClothes = [], selectedEmp = null) {
  // שלב 1: מניעת סריקה כפולה בסשן
  if (scannedClothes.find(c => c.barcode === barcode)) {
    return { ok: false, type: "warning", msg: `⚠️ הברקוד ${barcode} כבר נסרק בסשן זה!`, cloth: null, employee: null };
  }

  // שלב 2: האם הברקוד קיים?
  ...
  if (mode === "handout") {
    ...
    // שלב 3: הגבלת 5 פריטים מאותה קטגוריה לעובד
    if (selectedEmp) {
      const existingOfType = clothes.filter(c => c.status === "נמסר לעובד" && c.assignedTo === selectedEmp.id && c.type === cloth.type).length;
      const sessionOfType = scannedClothes.filter(c => c.type === cloth.type).length;
      if (existingOfType + sessionOfType >= MAX_PER_CATEGORY) {
        return {
          ok: false, type: "blocked",
          msg: `⛔ לא ניתן למסור. ${selectedEmp.name} כבר מחזיק ${existingOfType + sessionOfType} ${cloth.type} (מקסימום ${MAX_PER_CATEGORY}).`,
          cloth, employee: selectedEmp
        };
      }
    }
    return { ok: true, type: "success", msg: `✓ ${cloth.type} (${cloth.barcode}) – ${cloth.size}`, cloth, employee: null };
  }
...
export default function App() {
  const [employees, setEmployees] = useLS("ww5_employees", INITIAL_EMPLOYEES);
  const [clothes, setClothes] = useLS("ww5_clothes", INITIAL_CLOTHES);
  const [transactions, setTransactions] = useLS("ww5_transactions", INITIAL_TRANSACTIONS);
  const [auditLog, setAuditLog] = useLS("ww5_audit", []);
    ...
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #1A2332 0%, #2D3E55 100%)", direction: "rtl" }}>
      <div style={{ background: "white", borderRadius: 16, padding: 40, width: 360, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input value={u} onChange={e => setU(e.target.value)} placeholder="shimi / manager" style={{ ...inputStyle, width: "100%", boxSizing: "border-box" }} />
        </div>
...
function EmployeesPage({ employees, setEmployees, clothes, transactions, nextId, isEditor, addAudit, currentUser }) {
  ...
  const openAdd = () => { setForm({ employeeNum: "", uid: "", name: "", department: "", phone: "", type: "permanent", status: "active" }); setEditing(null); setShowForm(true); };
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {[["list","רשימה"],["spreadsheet","עריכת טבלה"]].map(([id,l]) => (
          <button key={id} onClick={() => setTab(id)} style={{ padding: "7px 16px", borderRadius: 8, border: "1.5px solid", cursor: "pointer", fontSize: 13, fontWeight: 600, background: tab===id?"#FF6B35":"white", borderColor: tab===id?"#FF6B35":"#E5E7EB", color: tab===id?"white":"#374151" }}>{l}</button>
        ))}
      </div>

      {tab === "spreadsheet" && isEditor && (
        <div style={card}>
          <h3 style={cardTitle}>עריכת עובדים – טבלה</h3>
          <SpreadsheetEditor
            data={employees}
            columns={empColumns}
            onSave={(row, isNew) => {
              if (isNew) {
                setEmployees(prev => [...prev, { ...row, id: nextId(prev), status: row.status||"active", type: row.type||"permanent" }]);
                addAudit("הוספת עובד (טבלה)", row.name||"", currentUser.id);
              } else {
                setEmployees(prev => prev.map(e => e.id === row.id ? {...e, ...row} : e));
                addAudit("עריכת עובד (טבלה)", row.name||"", currentUser.id);
              }
            }}
            onDelete={(row) => { if(window.confirm("למחוק את " + row.name + "?")) setEmployees(prev => prev.filter(e => e.id !== row.id)); }}
            newRowTemplate={{ employeeNum:"", uid:"", name:"", department:"", phone:"", type:"permanent", status:"active" }}
          />
        </div>
      )}
      ...
      {tab === "list" && showForm && (
        <Modal title={editing ? "עריכת עובד" : "הוספת עובד"} onClose={() => setShowForm(false)} onSave={save}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            ...
            <Field label='מזהה (3 אותיות) *' value={form.uid || ""} onChange={v => setForm(p => ({...p, uid: v.toUpperCase().slice(0,3)}))} placeholder="MSH" />
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        {isEditor && <ExcelImportButton onImport={(rows) => {
          const newEmps = rows.map((r, i) => ({
            id: nextId(employees) + i,
            employeeNum: r["מספר עובד"] || r["employeeNum"] || "",
            uid: (r["מזהה"] || r["uid"] || "").toUpperCase().slice(0,3),
            name: r["שם"] || r["name"] || "",
            department: r["מחלקה"] || r["department"] || "",
            phone: r["טלפון"] || r["phone"] || "",
            type: r["סוג"] === "זמני" ? "temporary" : "permanent",
            status: "active"
          })).filter(e => e.name);
            ...
            {filtered.map((emp, i) => {
              ...
              return (
                <tr key={emp.id} style={{ background: i % 2 === 0 ? "white" : "#FAFAFA" }}>
                  ...
                  <td style={tdStyle}><code style={{...codePill, background:"#E0E7FF", color:"#3730A3"}}>{emp.uid || "–"}</code></td>
...
function ClothesPage({ clothes, setClothes, employees, transactions, nextId, isEditor, addAudit, currentUser }) {
  const [clothTab, setClothTab] = useState("list");
  const [search, setSearch] = useState("");
  ...
  const clothColumns = [
    ...
    { key: "type", label: "סוג", options: ["חולצה","מכנס","נעליים"] },
    { key: "size", label: "מידה" },
    { key: "status", label: "סטטוס", options: ["במלאי","נמסר לעובד","תקול","אבוד","הושמד"] },
    { key: "washCount", label: "כביסות" },
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {[["list","רשימה"],["spreadsheet","עריכת טבלה"]].map(([id,l]) => (
          <button key={id} onClick={() => setClothTab(id)} style={{ padding: "7px 16px", borderRadius: 8, border: "1.5px solid", cursor: "pointer", fontSize: 13, fontWeight: 600, background: clothTab===id?"#FF6B35":"white", borderColor: clothTab===id?"#FF6B35":"#E5E7EB", color: clothTab===id?"white":"#374151" }}>{l}</button>
        ))}
      </div>

      {clothTab === "spreadsheet" && isEditor && (
        <div style={card}>
          <h3 style={cardTitle}>עריכת בגדים – טבלה</h3>
          <SpreadsheetEditor
            data={clothes}
            ...
            onSave={(row, isNew) => {
              if (isNew) {
                ...
                const dup = clothes.find(c => c.barcode === row.barcode);
                if (dup) { alert("ברקוד " + row.barcode + " כבר קיים!"); return; }
                setClothes(prev => [...prev, { ...row, id: nextId(prev), washCount: +row.washCount||0, status: row.status||"במלאי", assignedTo: null }]);
                addAudit("הוספת בגד (טבלה)", row.barcode, currentUser.id);
              } else {
                setClothes(prev => prev.map(c => c.id === row.id ? {...c, ...row, washCount: +row.washCount||0} : c));
                addAudit("עריכת בגד (טבלה)", row.barcode, currentUser.id);
              }
            }}
            onDelete={(row) => { if(window.confirm("למחוק בגד " + row.barcode + "?")) setClothes(prev => prev.filter(c => c.id !== row.id)); }}
            newRowTemplate={{ barcode:"", type:"חולצה", size:"", status:"במלאי", washCount:0, purchaseDate: new Date().toISOString().split("T")[0] }}
          />
        </div>
      )}

      {clothTab === "list" && showForm && (
...
function HandoutPage({ employees, setEmployees, clothes, setClothes, transactions, setTransactions, nextId, currentUser, addAudit }) {
  ...
  const handleScan = (barcode) => {
    const result = validateScan(barcode, clothes, employees, transactions, "handout", scannedClothes, selectedEmp);
...
function ReturnsPage({ clothes, setClothes, transactions, setTransactions, employees, nextId, currentUser, addAudit }) {
  ...
  const handleScan = (barcode) => {
    const result = validateScan(barcode, clothes, employees, transactions, "returns", scannedReturns);
    setLastResult(result);
  ...
  const finishSession = () => {
    ...
    setClothes(prev => prev.map(c => {
      ...
      return r ? { ...c, status: "במלאי", assignedTo: null, washCount: c.washCount + 1 } : c;
    }));
      ...
      {tab === "daily" && (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <button onClick={() => exportToCSV("דוח-יומי", ["עובד","מחלקה","סוג בגד","ברקוד"],
              missing.flatMap(({emp,items}) => items.map(c => [emp.name, emp.department, c.type+" "+c.size, c.barcode]))
            )} style={btnSecondary}>📊 ייצוא CSV</button>
            <button onClick={() => {
              exportToPrint("דוח יומי – חוסרים",
                ["עובד","מחלקה","סוג בגד","ברקוד"],
                missing.flatMap(({emp,items}) => items.map(c => [emp.name, emp.department, c.type+" "+c.size, c.barcode]))
              );
            }} style={btnSecondary}>🖨️ PDF</button>
          </div>
      ...
      {tab === "history" && (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button onClick={() => exportToCSV("היסטוריה", ["תאריך","עובד","ברקוד","סוג","פעולה"],
              [...transactions].reverse().map(t => {
                const emp = employees.find(e=>e.id===t.employeeId);
                const c = clothes.find(x=>x.id===t.clotheId);
                return [new Date(t.datetime).toLocaleString("he-IL"), emp?.name||"", c?.barcode||"", c?.type||"", t.action];
              })
            )} style={btnSecondary}>📊 ייצוא CSV</button>
            <button onClick={() => {
              exportToPrint("היסטוריית תנועות",
                ["תאריך","עובד","ברקוד","סוג","פעולה"],
                [...transactions].reverse().map(t => {
                  const emp = employees.find(e=>e.id===t.employeeId);
                  const c = clothes.find(x=>x.id===t.clotheId);
                  return [new Date(t.datetime).toLocaleString("he-IL"), emp?.name||"", c?.barcode||"", c?.type||"", t.action];
...
function ExcelImportButton({ onImport, label = "ייבוא מאקסל" }) {
  const ref = useRef(null);
  const handleFile = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (!XLSX) { alert("ספריית Excel לא זמינה"); return; }
  ...
  return (
    <div>
      ...
      <button onClick={() => ref.current?.click()} style={{ ...btnSecondary, gap: 6 }}>
        📊 {label}
      </button>
    </div>
  );
...
function exportToPrint(title, headers, rows) {
  ...
  win.document.write("<!DOCTYPE html><html dir=\"rtl\"><head><meta charset=\"utf-8\"><title>" + title + "</title><style>body{font-family:Arial,sans-serif;direction:rtl;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:right}th{background:#f0f0f0;font-weight:bold}h2{color:#1A2332}p{color:#6B7280;font-size:13px}@media print{.noprint{display:none}}</style></head><body><h2>" + title + "</h2><p>תאריך: " + date + " | סה\"כ שורות: " + rows.length + "</p>" + tableHtml + "<br><button class=\"noprint\" onclick=\"window.print()\" style=\"padding:10px 20px;background:#FF6B35;color:white;border:none;border-radius:8px;cursor:pointer;font-size:15px\">🖨️ הדפס / שמור כ-PDF</button></body></html>");
  win.document.close();
...
// ===== SPREADSHEET EDITOR =====
function SpreadsheetEditor({ data, columns, onSave, onDelete, newRowTemplate }) {
  const [rows, setRows] = useState(data.map(r => ({...r, _editing: false})));
  const [newRow, setNewRow] = useState(null);

  useEffect(() => { setRows(data.map(r => ({...r, _editing: false}))); }, [data]);
  ...
  const saveRow = (idx) => {
    ...
    delete row._editing;
    onSave(row);
    setRows(prev => prev.map((r, i) => i === idx ? {...r, _editing: false} : r));
  };
  ...
  const exportCSV = () => {
    ...
    const rowsCSV = rows.map(r => columns.map(c => `"${String(r[c.key] || "").replace(/"/g, '""')}"`).join(",")).join("\n");
  ...
  const importCSV = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      imported.forEach(row => onSave(row, true));
      alert("יובאו " + imported.length + " שורות");
    };
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <button onClick={addNew} style={btnPrimary}><Icon name="plus" size={15} color="white" /> הוסף שורה</button>
        <button onClick={exportCSV} style={btnSecondary}>📊 ייצוא CSV/Excel</button>
        <button onClick={() => fileRef.current?.click()} style={btnSecondary}>📥 ייבוא מ-CSV/Excel</button>
        <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls" style={{ display:"none" }} onChange={importCSV} />
        <span style={{ fontSize: 11, color: "#9CA3AF", alignSelf: "center" }}>לייבוא מאקסל: שמור קובץ כ-CSV ואז ייבא</span>
      </div>

      {newRow && (
        <div style={{ display: "flex", gap: 6, marginBottom: 8, padding: "10px", background: "#F0FDF4", borderRadius: 8, border: "1px solid #BBF7D0", flexWrap: "wrap" }}>
          {columns.map(col => (
            <div key={col.key} style={{ flex: 1, minWidth: 80 }}>
              <div style={{ fontSize: 10, color: "#6B7280", marginBottom: 2 }}>{col.label}</div>
              {col.options ? (
                <select value={newRow[col.key] || ""} onChange={e => setNewRow(p => ({...p, [col.key]: e.target.value}))} style={{ ...inputStyle, padding: "4px 6px", fontSize: 12 }}>
                  {col.options.map(o => <option key={o}>{o}</option>)}
                </select>
              ) : (
                <input value={newRow[col.key] || ""} onChange={e => setNewRow(p => ({...p, [col.key]: e.target.value}))} style={{ ...inputStyle, padding: "4px 6px", fontSize: 12 }} />
              )}
            </div>
          ))}
          <div style={{ display: "flex", alignItems: "flex-end", gap: 4 }}>
            <button onClick={saveNew} style={{ ...btnPrimary, background: "#10B981", padding: "6px 12px", fontSize: 12 }}>✓</button>
            <button onClick={() => setNewRow(null)} style={{ ...btnSecondary, padding: "6px 12px", fontSize: 12 }}>✕</button>
          </div>
        </div>
      )}
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ ...tableStyle, fontSize: 12 }}>
          <thead>
            <tr style={{ background: "#F9FAFB" }}>
              {columns.map(col => <th key={col.key} style={{ ...thStyle, fontSize: 11, whiteSpace: "nowrap" }}>{col.label}</th>)}
              <th style={thStyle}>פעולות</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, idx) => (
              <tr key={row.id || idx} style={{ background: idx % 2 === 0 ? "white" : "#FAFAFA" }}>
                {columns.map(col => (
                  <td key={col.key} style={{ ...tdStyle, padding: "6px 10px" }}>
                    {row._editing ? (
                      col.options ? (
                        <select value={row[col.key] || ""} onChange={e => updateCell(idx, col.key, e.target.value)} style={{ ...inputStyle, padding: "3px 6px", fontSize: 12, width: "100%" }}>
                          {col.options.map(o => <option key={o}>{o}</option>)}
                        </select>
                      ) : (
                        <input value={row[col.key] || ""} onChange={e => updateCell(idx, col.key, e.target.value)} style={{ ...inputStyle, padding: "3px 6px", fontSize: 12, minWidth: 60 }} />
                      )
                    ) : (
                ...
                <td style={{ ...tdStyle, padding: "6px 10px", whiteSpace: "nowrap" }}>
                  {row._editing ? (
                    <>
                      <button onClick={() => saveRow(idx)} style={{ background: "#10B981", border: "none", borderRadius: 5, padding: "3px 8px", color: "white", cursor: "pointer", fontSize: 12, marginLeft: 4 }}>שמור</button>
                      <button onClick={() => setRows(prev => prev.map((r,i) => i===idx ? {...r, _editing: false} : r))} style={{ background: "#6B7280", border: "none", borderRadius: 5, padding: "3px 8px", color: "white", cursor: "pointer", fontSize: 12 }}>ביטול</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => setRows(prev => prev.map((r,i) => i===idx ? {...r, _editing: true} : r))} style={iconBtn}><Icon name="edit" size={13} color="#6B7280" /></button>
                      {onDelete && <button onClick={() => onDelete(row)} style={{ ...iconBtn, marginLeft: 0 }}><Icon name="x" size={13} color="#EF4444" /></button>}
                    </>
                  )}
                </td>
              </tr>
        ...
        {rows.length === 0 && <Empty text="אין נתונים" />}
      </div>
    </div>
  );
}
