      ...
      {tab === "daily" && (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            ...
            <button onClick={() => {
              const rows = missing.flatMap(({emp,items}) => items.map(c =>
                "<tr><td>" + emp.name + "</td><td>" + emp.department + "</td><td>" + c.type + " " + c.size + "</td><td>" + c.barcode + "</td></tr>"
              )).join("");
              exportToPrint("דוח יומי – חוסרים", "<table><tr><th>עובד</th><th>מחלקה</th><th>סוג בגד</th><th>ברקוד</th></tr>" + rows + "</table>");
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            <button onClick={() => {
              const rows = [...transactions].reverse().map(t => {
                const emp = employees.find(e=>e.id===t.employeeId);
                const c = clothes.find(x=>x.id===t.clotheId);
                return "<tr><td>" + new Date(t.datetime).toLocaleString("he-IL") + "</td><td>" + (emp?.name||"") + "</td><td>" + (c?.barcode||"") + "</td><td>" + (c?.type||"") + "</td><td>" + t.action + "</td></tr>";
              }).join("");
              exportToPrint("היסטוריית תנועות", "<table><tr><th>תאריך</th><th>עובד</th><th>ברקוד</th><th>סוג</th><th>פעולה</th></tr>" + rows + "</table>");
            }} style={btnSecondary}>🖨️ ייצוא PDF</button>
          </div>
        <div style={tableWrap}>
          <table style={tableStyle}>
