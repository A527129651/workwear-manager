...
function EmployeeReport({ employees, clothes, transactions }) {
  ...
  return (
    <div>
      ...
      {emp && (
        <div style={card}>
          <div style={{ display: "flex", gap: 20, marginBottom: 16, padding: "12px", background: "#F9FAFB", borderRadius: 8 }}>
            <div><div style={{ fontSize: 10, color: "#6B7280" }}>שם</div><div style={{ fontWeight: 700, fontSize: 13 }}>{emp.name}</div></div>
            <div><div style={{ fontSize: 10, color: "#6B7280" }}>מחלקה</div><div style={{ fontWeight: 700, fontSize: 13 }}>{emp.department || "–"}</div></div>
            <div><div style={{ fontSize: 10, color: "#6B7280" }}>ברשותו כעת</div><div style={{ fontWeight: 700, fontSize: 13, color: currentItems.length > 0 ? "#1D4ED8" : "#9CA3AF" }}>{currentItems.length} בגדים</div></div>
            <div><div style={{ fontSize: 10, color: "#6B7280" }}>סה״כ תנועות</div><div style={{ fontWeight: 700, fontSize: 13 }}>{empTx.length}</div></div>
          </div>
          {currentItems.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 6, color: "#1D4ED8" }}>בגדים ברשותו כעת:</div>
              {currentItems.map(c => <div key={c.id} style={{ fontSize: 12, padding: "5px 10px", background: "#EFF6FF", borderRadius: 6, marginBottom: 4, display: "flex", justifyContent: "space-between" }}><span>{c.type} {c.size} {c.color}</span><code style={{ fontSize: 11 }}>{c.barcode}</code></div>)}
            </div>
          )}
          <table style={tableStyle}>
